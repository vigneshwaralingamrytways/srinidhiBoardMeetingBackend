...
public class CustomLogoutSuccessHandler implements LogoutHandler {
	 ...
	 * jwt; if (authHeader == null ||!authHeader.startsWith("Bearer ")) { return; }
	 * 
	 * jwt = authHeader.substring(7); System.out.println("jwt"+jwt);
	 * 
	 * 
			...
			if (users.isPresent()) {
				...
				try {
					new ObjectMapper().writeValue(response.getOutputStream(),
							new String("user-> " + username + " Logged out sucessfully"));
				} catch (IOException e) {
